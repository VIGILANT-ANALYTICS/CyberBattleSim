...
class EnvironmentBounds(NamedTuple):
    ...
    def of_identifiers(
        ...
        maximum_discoverable_credentials_per_action = (
            maximum_discoverable_credentials_per_action or maximum_total_credentials
        )

        assert np.can_cast(
            maximum_total_credentials, np.int32
        ), "maximum_total_credentials must be a 32-bit integer"
        assert np.can_cast(
            maximum_node_count, np.int32
        ), "maximum_node_count must be a 32-bit integer"
        assert (
            maximum_total_credentials > 0
        ), "maximum_total_credentials must be positive"
        assert maximum_node_count > 0, "maximum_node_count must be positive"
        assert np.can_cast(
            len(identifiers.ports), np.int32
        ), "port_count must be a 32-bit integer"
        assert np.can_cast(
            len(identifiers.properties), np.int32
        ), "property_count must be a 32-bit integer"
        assert np.can_cast(
            len(identifiers.local_vulnerabilities), np.int32
        ), "local_attacks_count must be a 32-bit integer"
        assert np.can_cast(
            len(identifiers.remote_vulnerabilities), np.int32
        ), "remote_attacks_count must be a 32-bit integer"
        assert np.can_cast(
            maximum_discoverable_credentials_per_action, np.int32
        ), "maximum_discoverable_credentials_per_action must be a 32-bit integer"
        ...
        super().__init__(
            {
                ...
                "discovered_nodes_properties": spaces.MultiDiscrete(
                    np.full(
                        shape=(bounds.maximum_node_count, bounds.property_count),
                        fill_value=3,
                    )
                ),
...
class CyberBattleEnv(CyberBattleSpaceKind):
    ...
    def __init__(
        ...
        action_spaces = {
            "local_vulnerability": spaces.MultiDiscrete(
                ...
                np.array(
                    [maximum_node_count_int32, local_vulnerabilities_count],
                    dtype=np.int32,
                )
            ),
    ...
    def __get_blank_action_mask(self) -> ActionMask:
        ...
        port_count = self.__bounds.port_count
        local = numpy.zeros(
            shape=(max_node_count, local_vulnerabilities_count), dtype=numpy.int8
        )
        remote = numpy.zeros(
            ...
            if self.__agent_owns_node(source_node_id):
                ...
                for target_node_id in self.__discovered_nodes:
                    ...
                    for vulnerability_index in range(remote_vulnerabilities_count):
                        vulnerability_id = self.__index_to_remote_vulnerabilityid(
                            vulnerability_index
                        )
                        node_vulnerable = (
                            vulnerability_id in self.__environment.vulnerability_library
                            or vulnerability_id
                            in self.__environment.get_node(
                                target_node_id
                            ).vulnerabilities
                        )
    ...
    def __get_property_matrix(self) -> numpy.ndarray:
        ...
        as_numpy = numpy.array(
            self.__pad_tuple_if_requested(
                property_discovered,
                self.__bounds.property_count,
                self.__bounds.maximum_node_count,
            )
        )
        assert as_numpy.shape == (
            self.__bounds.maximum_node_count,
            self.__bounds.property_count,
        )
        return as_numpy

    def get__owned_nodes_indices(self) -> List[int]:
    ...
    def get_all_valid_actions(self):
        ...
        results = []
        for action_type, mask in action_mask.items():
            indices = np.argwhere(mask == 1)
            for idx in indices:
                results.append({action_type: np.array(idx, dtype=np.int32)})
        return results
